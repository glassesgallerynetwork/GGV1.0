...
class Order extends Base {
    ...
    public function order_detail(){
        ...
        if($order_info['prom_type'] == 4){
            $pre_sell_item =  Db::name('goods_activity')->where(array('act_id'=>$order_info['prom_id']))->find();
            $pre_sell_item = array_merge($pre_sell_item,unserialize($pre_sell_item['ext_info']));
        ...
        $region_list = Db::name('region')->where("id in (".$ids.")")->getField("id,name");
        $invoice_no = Db::name('DeliveryDoc')->where("order_id", $id)->getField('invoice_no',true);
    ...
    public function cancel_order_info(){
    	$order_id = I('order_id/d',0);
    	$order = Db::name('order')->where(array('order_id'=>$order_id,'order_status'=>3,'pay_status'=>1))->find();
