...
class CartLogic extends Model
{
        ...
        if ($goodsPromFactory->checkPromType($prom_type)) {
            ...
            if($this->goods['prom_type'] == 0){
                if (!empty($this->goods['price_ladder'])){
                    ...
                    $buyGoods['goods_price'] = $buyGoods['member_goods_price']  = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $buyGoods['shop_price'], $this->goods['price_ladder']);
    ...
    private function addNormalCart(){
        ...
        if ($userCartGoods) {
            ...
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
                $price = $goodsLogic->getGoodsPriceByLadder($userWantGoodsNum, $this->goods['shop_price'], $this->goods['price_ladder']);
            }
            ...
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
                $price = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $this->goods['shop_price'], $this->goods['price_ladder']);
            }
            $cartAddData = array(
...
//        $cartCheckAfterList = $cartList;
    ...
    public function changeNum($cart_id, $goods_num){
        ...
        if($cart->prom_type == 0 && !empty($cartGoods['price_ladder'])){
            //如果有阶梯价格,就是用阶梯价格
            $goodsLogic = new GoodsLogic();
            $cart->goods_price = $cart->member_goods_price = $goodsLogic->getGoodsPriceByLadder($goods_num, $cartGoods['shop_price'], $cartGoods['price_ladder']);
        }
        $cart->save();
